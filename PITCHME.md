...
対応プラットフォームは、iOS,Androidは勿論のこと、Androidの次期OSとも非公式に噂されるFuchciaにも対応してます。  
(FuchiahaはFlutterSDKが使用されている）
---
### Hello world
```java
...
並べると、機能的には同じですがそれぞれのOSっぽい感じになっている事が見えます。　　
...
class RandomWordsState extends State<RandomWords> {
  ...
  Widget _buildRow(WordPair pair) {
    ...
    return new ListTile(
      ...
      trailing: new Icon(
        alreadySaved ? Icons.cloud_download : Icons.cloud_queue,
...
### まとめ・感想
---
- Dartという専用言語の為、どうしても学習コストは高そう  
- インストールからアプリ作成Tutorialは結構親切  
  （一部初期インストール時の設定値では動かない部分はありましたが...)  
- IDEプラグインによるデバックのし安さはよい  
- Fuchiaがどんどん持ち上がってくるなら、先行投資としては有能？  
---
#### Flutter  
Official  
https://flutter.io/  
